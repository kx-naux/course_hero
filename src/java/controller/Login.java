...
public class Login extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (hashedInputPassword.equals(accountRetrieved.getSaltedpassword())) {
            ...
            else if(userData.getUsertype().equals("Admin") || userData.getUsertype().equals("Manager") ){
    ...
    public static void getUserWishlist(HttpServletRequest request, EntityManager em, Users currentUser) {
        List<Wishlist> userWishlist = em.createNamedQuery("Wishlist.findByUserId").setParameter("userId", currentUser).getResultList();
        HttpSession session = request.getSession();

        List<Courses> courseWishlist = new ArrayList<>();
        
        for (Wishlist eachWish : userWishlist) {
            List<Courses> rawCourses = em.createNamedQuery("Courses.findByProductId").setParameter("productId", eachWish.getProductId()).getResultList();
            courseWishlist.add(rawCourses.get(0));
        }

        session.setAttribute("courseWishlist", courseWishlist);
    }
    ...
    public static void getUserCart(HttpServletRequest request, EntityManager em, Users currentUser){
        ...
        session.setAttribute("userCartCourse", userCartCourse);
        session.setAttribute("userCartMerch", userCartMerch);
        session.setAttribute("userCartTotal", userCartTotal);
    }
}
